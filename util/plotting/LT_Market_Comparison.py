...
def comparison(plot_inputs, plot_params_dict, ob_path, ticker, date1, startTime, endTime, title=None, out_file="liquidity_telemetry.png"):
    ...
    simulated_percentages = calculate_percentage_change(simulated_data)
    real_percentages = calculate_percentage_change(real_data)

    print(simulated_percentages)
    ...
    if rmse < .2:
        print("High Accuracy.")
    elif (rmse >= .4) and (rmse < .6): #1.0 <= rmse < 5.0:
