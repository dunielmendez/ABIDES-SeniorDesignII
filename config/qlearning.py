...
def get_rand_obj(seed_obj):
  return np.random.RandomState(seed = seed_obj.randint(low = 0, high = 2**32, dtype='uint64'))
...
kernel_seeds = np.random.RandomState(seed=np.random.randint(low=0,high=2**32, dtype='uint64'))

symbol_seeds = {}
for sym in symbols:  symbol_seeds[sym] = np.random.RandomState(seed=np.random.randint(low=0,high=2**32, dtype='uint64'))

agent_seeds = [ np.random.RandomState(seed=np.random.randint(low=0,high=2**32, dtype='uint64')) ] * num_agents
